...
// Índices cacheados para evitar COLS.indexOf() en cada iteración
const IDX = {};
...
function crearReserva(sheet, r) {
  ...
  try {
    ...
    if (!r.cabaña || !r.entrada || !r.salida || !r.nombre || !r.estado) {
      return buildResponse({ ok: false, error: 'Faltan campos obligatorios: cabaña, entrada, salida, nombre, estado.' });
    }
    const rows = sheet.getDataRange().getValues();
    for (let i = 1; i < rows.length; i++) {
      const row = rows[i];
      if (!row[0]) continue;
      const cab = row[IDX['Cabaña']];
      const ent = (row[IDX['Entrada']] || '').toString();
      const sal = (row[IDX['Salida']]  || '').toString();
      if (cab === r.cabaña && r.entrada < sal && r.salida > ent && r.entrada !== sal) {
        return buildResponse({
          ok: false, conflicto: true,
          error: 'Conflicto: ' + cab + ' ya fue reservada por ' + row[IDX['Nombre']] + ' en esas fechas.'
        });
      }
    }
    ...
    CacheService.getScriptCache().remove('reservas_data');
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (cab === r.cabaña && r.entrada < sal && r.salida > ent && r.entrada !== sal) {
        return buildResponse({
          ok: false, conflicto: true,
          error: 'Conflicto: ' + cab + ' ya está reservada por ' + row[IDX['Nombre']] + ' en esas fechas.'
        });
      }
    }
    const noches     = Math.round((new Date(r.salida) - new Date(r.entrada)) / 86400000);
    const entradaStr = r.entrada.substring(0, 10);
    const salidaStr  = r.salida.substring(0, 10);
    const cargadaOriginal = (rows[rowIndex - 1][IDX['Cargada']] || '').toString();
    const updatedRow = [
      id, r.cabaña, r.tipo, r.estado, r.nombre, r.dni,
      r.tel||'', r.email||'', r.dir||'',
      entradaStr, salidaStr, noches,
...
function aplicarEstilosFila(sheet, rowIndex, estado, columnsCount) {
  ...
  const dateCols = [IDX['Entrada'] + 1, IDX['Salida'] + 1];
  ...
  const ec = sheet.getRange(rowIndex, IDX['Estado'] + 1);
